                    ...
                    for (ps[1] = 0; ps[1] <= (free_parameter_flags[1] ? tmax - tmin : 0) / h; ps[1]++)
                    {
                        ...
                        for (ps[0] = 0; ps[0] <= (free_parameter_flags[0] ? tmax - tmin : 0) / h; ps[0]++)
                        {
                            ...
                            foreach (var eq in equations)
                            {
                                foreach (var fs in eq.Partition)
                                {
                                    foreach (var v in from f in fs.Variables.Where(v => v.name != null)
                                                        select
    ...
    partial class Window
    {
        ...
        protected class Tensor : FormulaNodeContainer, IFormulaic, IVariable
        {
            ...
            public IFormulaic DeepCopy()
            {
                ...
                cout.parts.AddRange(from r in parts
                                    select
                    (from c in r
                     select c is IFormulaic ?
...
namespace Liber
{
 ...
 in ponti) { cout.Add(new PointF((float)p.rx, (float)p.ry)); }
        ...
        protected static class producti
        {
            ...
            public static O arrprd<I, O>(List<I> arr, Action<O, I> finc, O zero)
            {
                O cout = zero;
                try
                {
