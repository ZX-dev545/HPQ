...
namespace GCalc_Comp
{
    public partial class Window : Engine
    {
        ...
        {
            public static Font MathsFont = new Font("XITS", 10, FontStyle.Regular);
            public static Font DataFont = new Font("Arial", 9);

            public static string DisplayMaths = "Click-on-a-Tile";

            public static double rotation_speed = 0.03;
            public static double panning_speed = 0.05;

            public static Dictionary<string, Func<complex, IEnumerable<complex>>> functions;
            public static Dictionary<string, complex> variables;
        }
        ...
        class Tile : Control
        {
            ...
            public Tile(Control v, Point Location/*, formula L = null, formula R = null*/)
            {
                ...
                Canvas = new Panel();
                Canvas.Location = new Point(0, 0);// Location;
                Canvas.Name = "Canvas";
                Canvas.Size = Size;
                Canvas.TabIndex = 2;
                ...
                TextInput.Location = new Point(0, 0);// Location;
                ...
                PlotBtn = new Button();
                PlotBtn.Location = new Point(Width - 20, 0);
                PlotBtn.Name = "PlotBtn";
                PlotBtn.Size = Size;
                PlotBtn.Width = 20;
                PlotBtn.TabIndex = 2;
                PlotBtn.Click += OnPlotBtnClick;
                Controls.Add(PlotBtn);
            }
            ...
            private void OnClick(object sender, EventArgs e)
            {
                ...
                Globals.DisplayMaths = TextInput.Text;
            }
            ...
            private void OnChange(object sender, EventArgs e)
            {
                ...
                Globals.DisplayMaths = TextInput.Text;
                (Parent.Parent as Window).InputPane.Panel2.Invalidate(); //update formula viewer

                //make the next Tile when you start typing
                ...
                { //get rid of the second one if there are two empty tiles on the end
                    if (Parent.Controls.Count > 6 && ((Tile)Parent.Controls[Parent.Controls.Count - 1]).TextInput.Text == "" && ((Tile)Parent.Controls[Parent.Controls.Count - 2]).TextInput.Text == "")
                    {
                ...
                if((Parent.Controls.Count - 1) / 3 > 5)
                {
                    ...
                    */
                ((VScrollBar)Parent.Controls[0]).Maximum = (int)((Parent.Controls.Count - 1.0) * Size.Height * 10 / (1 * Parent.Height));
            ...
            /*
            Stack<Func<complex>> Interpret(string _line = "")
            {
                var OpStack = new Stack<Func<complex>>();

                int FindBracketPair(int j)
                {
                    var tab = 0;
                ...
                complex value(string obj)
                {
                    try
                    {
                        return Globals.variables[obj];
                    }
                    catch (IndexOutOfRangeException) { }
                    try
                    {
                        return Globals.functions[obj](;
                    }
                }
                
                if (_line == "") _line = TextInput.Text;
                ...
                for (int i = 0; i < line.Count; i++)
                {
                    if (line[i] == '(')
                    {
                        var j = FindBracketPair(i + 1);
                        OpStack.Concat(Interpret(_line.Substring(i + 1, j)));
                        line.RemoveAt(i);
                        i = j;
                        line.RemoveAt(i);
                    }
        ...
        void OnInputPanel2Paint(object sender, PaintEventArgs e)
        {
            try
            {
                var parser = new TexFormulaParser();
                var formula = parser.Parse(Globals.DisplayMaths);
                var bitmapsource = formula.RenderToPng(20.0, 0, 0, "Arial");//GetRenderer(TexStyle.Display, 1000, "Arial").RenderToBitmap(0, 0);
                //File.WriteAllBytes(Environment.CurrentDirectory + "\\cout.png", bitmapsource);
                var image = Image.FromStream(new MemoryStream(bitmapsource));// Image.FromFile(Environment.CurrentDirectory + "\\cout.png");
                e.Graphics.DrawImage
                    (
                    image,
                    (InputPane.Panel2.Width - image.Width) / 2,
                    (InputPane.Panel2.Height - image.Height) / 2,
                    Math.Min(image.Width, InputPane.Panel2.Width),
                    Math.Min(image.Height, InputPane.Panel2.Height)
                    );
            }
            ...
            using (MemoryStream sout = new MemoryStream())
            {
                ...
                enc.Save(sout);
                bitmap = new Bitmap(sout);
            }
            e.Graphics.DrawImage(bitmap, 0, 0, 100, 100);
            */
        }
